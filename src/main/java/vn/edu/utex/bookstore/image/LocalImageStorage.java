...
public class LocalImageStorage implements ImageStorage {
    ...
    public LocalImageStorage(Path root) { this.root = root.toAbsolutePath().normalize(); }
    public String store(InputStream input, long size) throws IOException {
        ...
        try (var imageInput = javax.imageio.ImageIO.createImageInputStream(new ByteArrayInputStream(data))) {
            ...
            try {
                ...
                if ((long)reader.getWidth(0) * reader.getHeight(0) > 20_000_000L) throw Problem.invalid("Ảnh vượt quá 20 megapixel.");
                if (reader.read(0) == null) throw Problem.invalid("Ảnh không hợp lệ.");
            } finally { reader.dispose(); }
        } catch (IOException e) { throw Problem.invalid("Ảnh bị hỏng hoặc không đúng định dạng."); }
    ...
    static String extension(byte[] b) {
        if (b.length >= 12 && b[0] == (byte)0xff && b[1] == (byte)0xd8 && b[2] == (byte)0xff) return "jpg";
        if (b.length >= 24 && Arrays.equals(Arrays.copyOf(b,8),new byte[]{(byte)137,80,78,71,13,10,26,10})) return "png";
        if (b.length >= 20 && new String(b,0,4,java.nio.charset.StandardCharsets.US_ASCII).equals("RIFF") &&
                new String(b,8,4,java.nio.charset.StandardCharsets.US_ASCII).equals("WEBP")) return "webp";
    ...
    public String validateReference(String raw) {
        ...
        try {
            ...
            if (value.length() <= 1000 && ("https".equalsIgnoreCase(uri.getScheme()) || "http".equalsIgnoreCase(uri.getScheme())) &&
    ...
    public Path find(String name) {
        ...
        if (!file.startsWith(root) || Files.isSymbolicLink(file) || !Files.isRegularFile(file)) throw Problem.missing();
        return file;
    }
}
