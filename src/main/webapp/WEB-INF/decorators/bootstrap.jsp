  ...
  <div class="account">
    <c:choose>
      <c:when test="${not empty identity}">
        <a href="<c:url value='/auth/profile'/>">Chào, <c:out value="${identity.username}"/></a>
        <form method="post" action="<c:url value='/auth/logout'/>"><input type="hidden" name="_csrf" value="<c:out value='${csrf}'/>"><button class="link-button" type="submit">Đăng xuất</button></form>
      </c:when>
      <c:otherwise><a class="btn btn-outline-dark btn-sm" href="<c:url value='/auth/login'/>">Đăng nhập ↗</a></c:otherwise>
    </c:choose>
  </div>
</header>
<main id="main"><sitemesh:write property="body"/></main>
<footer><a class="brand" href="<c:url value='/'/>">góc<span>sách.</span></a><p>Những cuốn sách hay luôn tìm được người đọc mới.</p><small>Bài tập Lập trình Web · Java / PostgreSQL</small></footer>
</body>
</html>
